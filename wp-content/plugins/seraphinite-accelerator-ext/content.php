	...
	else
	{
		add_action( 'wp_loaded',
			function()
			{
				...
				{
					$aTestRes = array();
					$aTestRes[ 'paths' ][ 'siteRootUrl' ] = Wp::GetSiteRootUrl();
					$aTestRes[ 'paths' ][ 'siteWpRoot1' ] = Wp::GetSiteWpRootUrl( '', null, true );
					$aTestRes[ 'paths' ][ 'siteWpRoot2' ] = Wp::GetSiteWpRootUrl();
					$aTestRes[ 'paths' ][ 'ABSPATH' ] = ABSPATH;
					$aTestRes[ 'paths' ][ 'WP_CONTENT_DIR' ] = WP_CONTENT_DIR;
					$aTestRes[ 'paths' ][ 'SERAPH_ACCEL_CACHE_DIR' ] = defined( 'SERAPH_ACCEL_CACHE_DIR' ) ? SERAPH_ACCEL_CACHE_DIR : null;
					$aTestRes[ 'ctx' ] = GetContentProcessCtx( $_SERVER, Plugin::SettGet() );
					$aTestRes[ 'postTypes' ] = get_post_types( array(), 'objects' );
					$aTestRes[ 'taxonomies' ] = get_taxonomies( array(), 'objects' );
					echo( Ui::Tag( 'style', 'pre{tab-size:4;}' ) . Ui::Tag( 'pre', htmlentities( json_encode( $aTestRes, JSON_PRETTY_PRINT ), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 ) ) );
...
function OnEarlyContentComplete( $buffer, $tmpUpdate = false )
{
	...
	if( Gen::StrStartsWith( $seraph_accel_g_prepCont, 'tmp' ) )
	{
		if( Gen::GetArrField( $settCache, array( 'fastTmpOpt' ), false ) )
		{
			...
			{
				if( !headers_sent() )
					header( 'X-Seraph-Accel-Content: 2.26.9; status=' . $skipStatus . ', procTime=' . $tmProc . 's' );
