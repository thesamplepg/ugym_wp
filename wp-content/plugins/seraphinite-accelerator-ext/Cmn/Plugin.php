...
class PluginRmtCfg
{
	...
	static function Update( $bForce = false, $bFirstTimeOnly = false )
	{
		...
		if( $lastCheckVer !== '2.26.9' || $lastCheckPackage !== 'Ext' )
		{
		...
		if( $isAdminMode )
		{
			...
			add_filter( 'plugin_action_links_' . Plugin::GetCurBaseName(),
				function( $actions, $plugin_file )
				{
					...
					if( version_compare( Gen::GetArrField( $rmtCfg, 'Info.LastVersion', '2.26.9' ), '2.26.9' ) > 0 )
					{
...
?>
	...
	static private function _TransientUpdatePlugins_Process( &$value, $plgVer, $switch )
	{
		...
		if( $switch )
			$dwnldUrl = Gen::GetFileName( $dwnldUrl, true, true ) . '.2.26.9.' . Gen::GetFileExt( $dwnldUrl );
		else
		{
		...
		if( ( $switch || version_compare( $verLast, '2.26.9' ) > 0 ) && !isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
		...
		if( isset( $value -> response[ Plugin::GetCurBaseName() ] ) )
		{
			...
			if( !$switch && version_compare( $verLast, '2.26.9' ) > 0 )
